...
const getRemainingTimeInMinutes = (hass, entity) => {
  ...
  // Convert hours to minutes if unit is 'h'
  if (unit === "h") {
    return Math.round(value * 60);
  }

  // Assume minutes if no unit or unit is 'min', 'm', etc.
...
export const getEntityStates = (hass, config) => {
  ...
  return {
    ...
    remainingTime: getRemainingTimeInMinutes(
      hass,
      config.remaining_time_entity
    ),
    ...
    aux_fan_entity:
      config.aux_fan_entity && hass.states[config.aux_fan_entity]
        ? config.aux_fan_entity
        : null,
